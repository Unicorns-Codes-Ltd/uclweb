<x-app-layout>
    ...
    <x-slot name="submenu">
        ...
        <div class="sm:gap-5  sm:ml-10 sm:flex">
            ...
            <x-nav-link :href="route('courses.edit',$course->id)" :active="request()->routeIs('courses.edit',$course->id)">
                {{ __('Edit This Course') }}
            </x-nav-link>
    ...
    <div class="p-2 sm:p-6">
        <div class="p-2 sm:p-6 bg-white rounded-md">
            <h1 class="text-xl mb-4 text-center sm:text-left">'{{$course->name}}' Course Information</h1>
            <p>(<span class="text-red-500 mb-4">*</span>) marked fields are required.</p>

            <form method="POST" action="{{ route('courses.update', $course->id ) }}" id="course-store" enctype="multipart/form-data" >
                @csrf
                @method('PATCH')
                <div class="grid grid-cols-2 gap-5">
                    ...
                    <div class="">
                        ...
                        value="{{$course->slug}}" required readonly />
                    ...
                    <div class="">
                        ...
                        <x-select-input id="category_id" name="category_id" required>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" class="capitalize" @if($category->id == $course->category_id) @selected(true) @endif>{{ $category->name }}</option>
                            @endforeach
                        </x-select-input>
                    </div>
                    ...
                    <div class="">
                        ...
                        <x-select-input id="instructor_id" name="instructor_id" required>
                            @foreach ($instructors as $instructor)
                                <option value="{{ $instructor->id }}" class="capitalize" @if($instructor->id == $course->instructor_id) @selected(true) @endif>{{ $instructor->name }}
                                </option>
                            @endforeach
                        </x-select-input>
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <x-textarea id="short_description" class="block mt-2 w-full" name="short_description" required
                            autocomplete="short_description">{!! $course->short_description !!}</x-textarea>
                    ...
                    <div class="flex justify-between gap-5">
                        ...
                        <div class="">
                            ...
                            <x-text-input id="regular_price" class="block mt-1 w-full" type="number" name="regular_price"
                                value="{{ $course->regular_price }}" required />
                        ...
                        <div class="">
                            ...
                            <x-text-input id="current_price" class="block mt-1 w-full" type="number" name="current_price"
                                value="{{ $course->current_price }}" required />
                        ...
                        <div class="">
                            ...
                            <x-select-input id="status" name="status" required>
                                    <option value="1" class="capitalize" @if($course->status == '1') @selected(true) @endif>Pending</option>
                                    <option value="2" class="capitalize" @if($course->status == '2') @selected(true) @endif>On Review</option>
                                    <option value="3" class="capitalize" @if($course->status == '3') @selected(true) @endif>Running</option>
                            </x-select-input>
                        </div>
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <x-textarea id="description" class="block mt-2 w-full" name="description" required
                            autocomplete="description">{!! $course->description !!}</x-textarea>
                    ...
                    <div class="col-span-2">
                        ...
                        <x-textarea id="materials" class="block mt-2 w-full" name="materials" required
                            autocomplete="materials">{!! $course->materials !!}</x-textarea>
                    ...
                    <div class="col-span-2">
                        ...
                        autocomplete="curriculam">{!! $course->curriculam !!}</x-textarea>
